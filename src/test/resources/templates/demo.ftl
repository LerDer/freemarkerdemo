<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <#if field.query>
      <# assign dictType = field.dictCode!>
      <# assign AttrName = field.fieldName!>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['${apiModuleName}:${apiName}:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['${apiModuleName}:${apiName}:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['${apiModuleName}:${apiName}:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['${apiModuleName}:${apiName}:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="${apiName}List" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['${apiModuleName}:${apiName}:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['${apiModuleName}:${apiName}:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="${apiName}Ref" :model="form" :rules="rules" label-width="80px">
      {% for column in columns %}
      ...
      {% if column.html_type == "input" %}
      <el-form-item v-if="renderField({{ column.insert | lower }}, {{ column.edit | lower }})" label="{{ comment }}" prop="${field.fieldName}">
        <el-input v-model="form.${field.fieldName}" placeholder="请输入{{ comment }}" />
      </el-form-item>
      ...
      <el-form-item v-if="renderField({{ column.insert | lower }}, {{ column.edit | lower }})" label="{{ comment }}" prop="${field.fieldName}">
        <el-select v-model="form.${field.fieldName}" placeholder="请选择{{ comment }}">
          <el-option
            v-for="dict in {{ dictType }}"
            :key="dict.value"
            :label="dict.label"
      ...
      <el-form-item v-if="renderField({{ column.insert | lower }}, {{ column.edit | lower }})" label="{{ comment }}" prop="${field.fieldName}">
        <el-input v-model="form.${field.fieldName}" type="textarea" placeholder="请输入内容" />
      </el-form-item>
      {% endif %}
      {% endif %}
      {% endif %}
...
import { list${classNamePrefix}, get${classNamePrefix}, del${classNamePrefix}, add${classNamePrefix}, update${classNamePrefix} } from "@/api/${apiModuleName}/${apiName}";
...
function getList() {
  ...
  {% endif %}
  {% endfor %}
  list${classNamePrefix}(queryParams.value).then(response => {
    ${apiName}List.value = response.rows;
...
function handleUpdate(row) {
  ...
  get${classNamePrefix}(_{{ pkColumn.python_field }}).then(response => {
    ...
    {% endif %}
    open.value = true;
    title.value = "修改${tableName}";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["${apiName}Ref"].validate(valid => {
    if (valid) {
      {% for column in columns %}
      {% if column.html_type == "checkbox" %}
      ...
      if (form.value.{{ pkColumn.python_field }} != null) {
        update${classNamePrefix}(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        add${classNamePrefix}(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除${tableName}编号为"' + _{{ pkColumn.python_field }}s + '"的数据项？').then(function() {
    return del${classNamePrefix}(_{{ pkColumn.python_field }}s);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('${apiModuleName}/${apiName}/export', {
    ...queryParams.value
